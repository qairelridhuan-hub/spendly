...
export default function AttendanceScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const workplace = result.ok ? result.workplace : {
      ...
      allowedRadiusMeters: 0, // fail-closed: block all if workplace fetch fails
    };
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: c.backgroundStart }]} edges={["top"]}>
      <ScrollView ref={scrollRef} contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.header}>
          ...
          <View style={[s.iconPill, pillExpanded ? { backgroundColor: c.surface, borderColor: c.border } : { backgroundColor: "transparent", borderColor: "transparent", shadowOpacity: 0, elevation: 0 }]}>
      ...
      <Modal transparent visible={showModal} animationType="none" onRequestClose={() => setShowModal(false)}>
        ...
        <Animated.View style={[s.modalSheet, { backgroundColor: c.surface, transform: [{ translateY: slideAnim }] }]}>
          ...
          {/* Status row */}
          <View style={[s.modalStatusRow, { backgroundColor: withinRadius ? "#16a34a12" : "#dc262612", borderWidth: 1, borderColor: withinRadius ? "#16a34a30" : "#dc262630" }]}>
            <View style={[s.modalStatusDot, { backgroundColor: withinRadius ? "#16a34a" : "#dc2626" }]} />
            <View style={{ flex: 1 }}>
              <Text style={[s.modalStatusText, { color: withinRadius ? "#16a34a" : "#dc2626", fontWeight: "700" }]}>
                {withinRadius ? "You are inside the workplace zone" : "You are outside the workplace zone"}
              </Text>
              {distanceMeters != null && (
                <Text style={{ fontSize: 11, color: c.textMuted, marginTop: 2 }}>
                  {`${distanceMeters}m from workplace · allowed radius: ${workplace?.allowedRadiusMeters ?? "—"}m`}
                </Text>
              )}
            </View>
            {withinRadius
              ? <CheckCircle2 size={16} color="#16a34a" strokeWidth={2} />
              : <View style={{ width: 16, height: 16, borderRadius: 8, borderWidth: 2, borderColor: "#dc2626" }} />
            }
          </View>
