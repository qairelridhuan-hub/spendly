            ...
            return (
              <View key={log.refPath} style={{ flexDirection: "row", paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: idx < sortedLogs.length - 1 ? 1 : 0, borderBottomColor: p.border, alignItems: "center" }}>
                ...
                <View style={{ flex: 1, gap: 3 }}>
                  ...
                  {log.locationStatus != null && (
                    <View style={{ backgroundColor: log.locationStatus === "inside" ? "#16a34a18" : "#dc262618", borderRadius: 99, paddingHorizontal: 6, paddingVertical: 1, alignSelf: "flex-start" }}>
                      <Text style={{ color: log.locationStatus === "inside" ? "#16a34a" : "#dc2626", fontSize: 9, fontWeight: "700" }}>{log.locationStatus === "inside" ? "Inside" : "Outside"}</Text>
                    </View>
                  )}
                </View>
        ...
        <View style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.5)", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <View style={{ width: "100%", maxWidth: 440, backgroundColor: p.surface, borderRadius: 14, borderWidth: 1, borderColor: p.border, overflow: "hidden" }}>
            ...
            <View style={{ padding: 16, gap: 8 }}>
              {selectedLog.locationStatus != null && (
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 6, paddingHorizontal: 10, borderRadius: 8, backgroundColor: selectedLog.locationStatus === "inside" ? "#16a34a12" : "#dc262612", borderWidth: 1, borderColor: selectedLog.locationStatus === "inside" ? "#16a34a30" : "#dc262630", marginBottom: 4 }}>
                  <Text style={{ color: p.textMuted, fontSize: 12 }}>Location status</Text>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    {selectedLog.distanceFromWorkplace != null && (
                      <Text style={{ color: p.textMuted, fontSize: 11 }}>{selectedLog.distanceFromWorkplace}m away</Text>
                    )}
                    <Text style={{ color: selectedLog.locationStatus === "inside" ? "#16a34a" : "#dc2626", fontSize: 12, fontWeight: "700" }}>{selectedLog.locationStatus === "inside" ? "Inside zone" : "Outside zone"}</Text>
                  </View>
                </View>
              )}
              {[
